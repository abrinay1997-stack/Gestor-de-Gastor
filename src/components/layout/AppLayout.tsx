  ...
  return (
    <div className="min-h-dvh flex flex-col md:flex-row">
      ...
      <nav className="md:hidden fixed inset-x-0 flex justify-center px-3 pointer-events-none z-30" style={{ bottom: 'calc(0.25rem + env(safe-area-inset-bottom, 0px))' }}>
        <div className={cn('barra-flotante pointer-events-auto flex w-full max-w-[430px] rounded-[32px] px-2.5 py-2', compacto && 'py-1.5')}>
          {enBarra.map((d) => (
            <button
              key={d.id}
              onClick={() => irA(d.id)}
              ...
              className={cn(
                'relative flex-1 flex flex-col items-center justify-center min-h-12 min-w-12 rounded-2xl py-2.5 gap-0.5 transition-[transform,opacity,padding] duration-[250ms] active:scale-[0.97]',
                solapa === d.id ? 'text-marca-600 dark:text-marca-500' : 'txt-3',
              )}
            >
              {solapa === d.id && <span aria-hidden="true" className="luz-activa" />}
              {solapa === d.id && (
                <span aria-hidden="true" className="absolute top-1 h-10 w-10 rounded-full bg-marca-500/20 blur-md dark:bg-marca-500/25" />
              )}
